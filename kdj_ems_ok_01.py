...
def get_data( begin_time, end_time, period ):
    ...
    df8 = tt.get_bars( "CZCE.CF709", 60, "2017-05-25 09:00:00", "2017-07-06 23:30:00" )
...
def strategy01( df ):
    ...
    for i in range( 1, len( df ) ):
        ...
        """
        if (buy == 0 and sell == 0):
            print("cu2----------cu2------------cu2 :",cu2)
            print("x2[i-1]",i,x2[i-1])
            print("x2[i]",i,x2[i])
        """
        if (cu2):
            start = df['close'].values[i]
            buy = 1
            sell = 0
            df['buy'].iat[i] = 1
            df['sell'].iat[i] = 0
            print( "buy ===================================11" )
            start_cash = initial_cash*df['gain'].values[i-1]
            print( "df['gain'].values[i-1]", i, df['gain'].values[i-1], initial_cash )
        ...
        if (buy == 1):
            sell = 0
            rate = (df['close'].values[i]-start)/start
            gain = start_cash*(1+rate)
            gr = gain/initial_cash
            if (x2[i]>0):
                df['gain'].iat[i] = gr
            if (x2[i]<=0):
                if (x3[i]<=0):
        ...
        if (sell == 1):
            buy = 0
            rate = (start-df['close'].values[i])/start
            gain = start_cash*(1+rate)
            gr = gain/initial_cash
