...
def strategy01( df ):
    ...
    for i in range( 610, len( df ) ):
        ...
        if (x2[i]==0):
            close_B(trading_data, df['close'].values[i])
            close_S(trading_data, df['close'].values[i])
            df['buy'].iat[i] = 0
            df['R'].iat[i] = trading_data["R"]
            df['BR'].iat[i] = trading_data["BR"]
...
def drawPic( df, code, name ):
    ...
    plt.plot( xx, df['diff4'], linewidth = 0.5 )
    plt.plot( xx, df['dea4'], linewidth = 0.5 )
    """
...
def makePicture( code, name ):
    ...
    #aa = sq.calc_macd1( df )
    aa = sq.calc_macd(df,1)
    aa = sq.calc_macd(df,2)
    aa = sq.calc_macd(df,3)
    aa = sq.calc_macd(df,4)

    print("xxxxx!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    #bb = stt.strategy01( df )
    bb = stt.strategy02( df )
    aa = strategy01( df )
    ...
    kdj_value = df[['close','j1','e1','j2','e2','j3','e3','j4','e4','jupd1','jpos1',
                    'jupd2', 'jpos2',
                    'jupd3', 'jpos3',
                    'jupd4', 'jpos4',]]
    kdj_value.to_csv("kdj_value.csv")
    """

    stt01 = df[['close','stt02_bs','BP', 'BR', 'R', 'buy']]
    stt01.to_csv("stt01.csv")
